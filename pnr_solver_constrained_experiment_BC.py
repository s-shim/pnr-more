...
for (numAlternatives,numOrigins) in [(30,100),(60,200),(90,300),(120,400)]:
    ...
    for instance in range(numInstances):
        ...
        print('### solve opt_constrainedBC_pnr_origin%s_I%s_J%s_p%s_inst%s.csv'%(numOrigins,numOrigins*numDestins,numAlternatives,numHubs,instance))
        ...
        def mycallback(model, where):
            if where == GRB.Callback.MIPSOL:
                ...
                for taz in subIndividuals['TAZ']:
                    for destin in range(numDestins):
                        totalOdds[taz,destin] = odds[taz,destin,0]
                        for j in tempSelected:
                            totalOdds[taz,destin] += odds[taz,destin,j]
                RHS = {}
                for taz in subIndividuals['TAZ']:
                    for destin in range(numDestins):
                        RHS[taz,destin] = ((totalOdds[taz,destin] - odds[taz,destin,0]) / totalOdds[taz,destin]) ** 2
                ...
                for taz in subIndividuals['TAZ']:
                    for destin in range(numDestins):
                        ...
                        for j in subAlternatives['id']:
                            ...
                            coeff = odds[taz,destin,j] / (totalOdds[taz,destin] ** 2)
                            LHS += [(- coeff,Y[j])]
                        model.cbLazy( LinExpr(LHS) <= RHS[taz,destin] )                                    
                        # model.cbCut( LinExpr(LHS) <= RHS[taz,destin] )                                    
                        numCutsAdded += 1
                print('numCutsAdded=',numCutsAdded)
                        

        # ILP Model
        model = Model('pnr')
        model.setParam('LogFile', 'log_constrainedBC_pnr_machine%s_origin%s_I%s_J%s_p%s_inst%s.txt'%(machineName,numOrigins,numOrigins*numDestins,numAlternatives,numHubs,instance))
        ...
        optSolution.to_csv(r'opt_constrainedBC_pnr_origin%s_I%s_J%s_p%s_inst%s.csv'%(numOrigins,numOrigins*numDestins,numAlternatives,numHubs,instance), index = False)#Check
        ...
        ilpTable = pd.DataFrame(list(zip(machineArray,originArray,iArray,jArray,pArray,instanceArray,ilpObjArray,accurateArray,unconstrainedArray,underArray,overArray,runArray,countArray)),columns =['Machine','Origins','I','J','p','instance','ILP OPT','Accuarate','ifUnconstrained','underCapacity','overCapacity','runTime','NodeCount'])
        ilpTable.to_csv(r'ilpTable_constrainedBC_pnr_origin%s_I%s_J%s_p%s.csv'%(numOrigins,numOrigins*numDestins,numAlternatives,numHubs), index = False)#Check
